...
import { renderProductControl } from './product-control.mjs';
...
document.addEventListener('submit', async event => {
  ...
  try {
    if (form.id === 'proposal-form') {
      ...
      await api(`/proposals/${form.dataset.id}/resolve`, { method: 'POST', body: { ...data, portalChecked: data.portalChecked === 'on' } }); await refresh(); render(); toast('处理结果已记录');
    } else if (form.id === 'product-control-form') {
      await api(`/products/${form.dataset.product}/availability`, { method: 'PATCH', body: { paused: data.paused === 'true', reason: data.reason, revision: Number(form.dataset.revision) } });
      await refresh(); render(); toast('产品执行状态已更新');
