...
工具／模型数字采用额外保守拦截，当前规则版本为 `m2a2-3`（新增香港身份证号识别，见文末补充）。没有经核验的官方数字来源，所以本里程碑
...
## 离线补充（2026-09-23）：HKID、按应用密钥与红线回归

仍全部离线：未连接 Dify、DeepSeek 或任何外网；HTTP 客户端只在测试中注入离线 transport。

**香港身份证号（规则版本 `m2a2-3`）。** `sensitive-data` 规则在内地 18 位身份证号之外，识别
1–2 个字母 + 6 位数字 + 校验位（0–9 或 A），校验位可带半角 ()、全角（）括号或不带括号；先做 NFKC
规范化，全角字母／数字同样覆盖。只有满足 mod-11 校验位算法的才算命中，以降低产品代码、保单号的误报；
无括号写法只认大写字母，并要求前后不紧邻字母或数字。同形且恰好校验通过的编号（约 1/11）仍会被拦截，
方向为转人工，属于可接受的保守误报。该规则同时作用于出口审查和助手／抽取输入检查（`SENSITIVE_INPUT`）。

**按应用区分 Dify 密钥。** `HttpDifyClient` 接收 `apiKeys: { chat, extract, compliance }`，分别对应
`broker_assistant_chat`、`proposal_extract`、`compliance_guard`。每项能力只用自己应用的密钥，
缺哪个应用的密钥，该能力就返回 `DIFY_APP_NOT_CONFIGURED`（503），不发请求，也不用其他应用的密钥代替；
单一共享密钥（`apiKey`）或多个应用使用同一密钥时拒绝创建。密钥保存在私有字段中，只出现在 Authorization
请求头，不进入请求正文、`status()`、错误信息或存储。所有调用使用 blocking 模式并带超时；传输失败返回
`DIFY_UNAVAILABLE`（502），不回显上游错误，也不换成本地结果。每次调用必须带后端维护的 `user`。
`status()` 按应用报告，部分配置时为 `partially-configured`。`reviewCompliance` 仅作建议：未知、异常或
不可用一律 block，改写结果要重新跑确定性规则；最终放行仍以后端确定性出口审查为准。
预留的环境变量名见 `DIFY_ENV_VARS`：`DIFY_API_URL`、`DIFY_CHAT_API_KEY`、`DIFY_EXTRACT_API_KEY`、
`DIFY_COMPLIANCE_API_KEY`（以及已废弃的 `DIFY_API_KEY`）。M2a-2 阶段应用入口已全部拒绝（`server/index.mjs`）。

**本地降级引擎与红线。** 产品事实问题的本地回答均含“无法核实”；参数卡回复说明利益数字以官方计划书为准。
本地有限规则抽取中，金额紧邻收益／回报／分红／现金价值等利益表述时，不作为年缴保费，改为冲突待经纪确认（RL-01d）。

**红线回归。** `tests/dify-redline.test.mjs` 按 [验收样例](handoff/m2b-m3/acceptance-cases.md) 编号覆盖
RL-01a–e、RL-02a/d、RL-03a/b/d、RL-04a/b/d/e、RL-05、RL-06、RL-07 的离线部分；RL-04c 的 blocking 契约与
C01／C03 见 `tests/dify-client.test.mjs`、`tests/dify-hkid.test.mjs`。这些只验证本地降级引擎、确定性守卫、
网关和客户端契约，不代表真实模型或 Dify 应用已通过验收。需要真实模型、知识库或 DSL 的用例
（RL-01f、RL-02b/c、RL-03c、RL-04a 真实探针应用、RL-04c 流式实现、C05–C07、K 系列、F05 等）仍待 M2b／M3 环境。

## M2b 开发联调编排补充（2026-09-24）

M2b 开发版由后端编排合规审计、计划书进度和跟进卡读取；三个 Dify workflow 不含后端工具节点，也不调用本文件的工具路由。助手出口在后端保存审计，计划书进度按当前经纪的租户与 owner 范围读取，跟进只读当前客户卡片；真实保单／理赔进度仍未接入。

这样做是因为 gateway 的短期能力令牌不能安全地传入 Dify：令牌可能进入 Dify 运行日志或模型输入。没有因此向模型／工作流提供令牌，也没有放宽签名、时间戳、nonce、令牌期限或资源范围检查。gateway 和回调状态机继续作为 M2a-2 的离线契约实现，M2b 不依赖它们触发 assistant 出口或推进 M1 任务。
